...
@patch("src.apps.chat.intent_service.litellm.completion")
def test_classify_query_intent_llm_vector_search(mock_completion):
    mock_resp = MagicMock()
...
@patch("src.apps.chat.intent_service.litellm.completion")
def test_classify_query_intent_llm_clarification(mock_completion):
    mock_resp = MagicMock()
    mock_resp.choices = [MagicMock(message=MagicMock(content=json.dumps({
...
@patch("src.apps.chat.intent_service.litellm.completion")
def test_classify_query_intent_llm_error_fallback(mock_completion):
    mock_completion.side_effect = Exception("API rate limit")
