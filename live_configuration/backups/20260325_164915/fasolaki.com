...
# -----------------------------------------------------------------------
# Growth Machine Django App — www.fasolaki.com/growth_machine/
# -----------------------------------------------------------------------

# Static files (collected via: python manage.py collectstatic)
	location /growth_machine/static/ {
		alias /srv/growth_machine/staticfiles/;
		expires 30d;
		access_log off;
		add_header Cache-Control "public, immutable";
	}

# User-uploaded media files (if used)
	location /growth_machine/media/ {
		alias /srv/growth_machine/media/;
		expires 7d;
		access_log off;
	}

# All dynamic requests proxied to gunicorn
	location /growth_machine/ {
		rewrite ^/growth_machine/(.*)$ /$1 break;
		rewrite ^/growth_machine/?$ / break;
		proxy_pass http://unix:/run/growth_machine/growth_machine.sock;
		proxy_set_header Host $host;
		proxy_set_header X-Real-IP $remote_addr;
		proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
		proxy_set_header X-Forwarded-Proto $scheme;
		proxy_set_header X-Forwarded-Prefix /growth_machine;
		proxy_redirect off;
