...
| **Command** | A user-facing entry point | `/my_review`, `/my_ship` | The *when* — composes personas and skills |
...
- `/my_review` → wraps `code-reviewer` with the project's review skill
- `/my_test` → wraps `test-engineer` with TDD skill
...
- `/my_ship` → fans out to `code-reviewer` + `security-auditor` + `test-engineer` in parallel, then synthesizes their reports into a go/no-go decision
...
└── No  → Are the sub-tasks independent (no shared mutable state, no ordering)?
         ├── Yes → Slash command with parallel fan-out (e.g. /my_ship)
         └── No  → Sequential slash commands run by the user (/my_spec → /my_plan → /my_build → /my_test → /my_review)
```

## Worked example: valid orchestration

`/my_ship` is the canonical fan-out orchestrator in this repo:
...
- The user already knows they want a review; let them call `/my_review` directly
...
- **As subagents:** auto-discovered when this plugin is enabled (no path config needed). Use the Agent tool with `subagent_type: code-reviewer` (or `security-auditor`, `test-engineer`). `/my_ship` is the canonical example.
